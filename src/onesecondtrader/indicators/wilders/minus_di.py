...
class MinusDI(indicators.IndicatorBase):
    """
    Negative Directional Indicator (-DI).

    Computes -DI using Wilder's smoothing method. One scalar value in the
    range 0-100 is produced per incoming bar and stored per symbol.
    ...
    def __init__(
        ...
        plot_color: models.PlotColor = models.PlotColor.RED,
    ) -> None:
        """
        Parameters:
            period:
    ...
    def name(self) -> str:
        ...
        """
        return f"MINUS_DI_{self.period}"
    ...
    def _compute_indicator(self, incoming_bar: events.market.BarReceived) -> float:
        ...
        _, minus_dm = _directional_movement(
        ...
        return 100.0 * self._smoothed_minus_dm[symbol] / smoothed_tr
