...
class BollingerUpper(indicators.IndicatorBase):
    """
    Bollinger Upper Band indicator.

    Computes the upper Bollinger Band as: SMA + ($k$ * standard deviation)
    ...
    def _compute_indicator(self, incoming_bar: events.market.BarReceived) -> float:
        ...
        Returns:
            Upper Bollinger Band value, or `numpy.nan` if the rolling window is not yet fully populated.
        """
